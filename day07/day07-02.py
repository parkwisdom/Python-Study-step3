...
def excelData02(): # 첫번째 시트만 보기
    ...
    workbook = xlrd.open_workbook(input_file)
    sheetCount = workbook.nsheets  # 속성
    sheet1 = workbook.sheets()[0] #0번째 시트만 가져오기
...
def excelData04(): #선택시트보기
    ...
    selectSheet = int(input("선택할 시트번호 ="))

    sheetCount = workbook.nsheets  # 속성
    sheet1 = workbook.sheets()[selectSheet]  # 0번째 시트만 가져오기
    sheetName = sheet1.name
    sRow = sheet1.nrows
    sCol = sheet1.ncols
    # print(sheetName, sRow, sCol)
...
excelMenu.add_command(label ='Excel 내용 보기-select',command = excelData04)
