...
def gammaImage(): #감마 알고리즘
    ...
    for i in range(outH):
        ...
        outImage.append(tmpList)
    #★★★★★진짜 영상처리 알고리즘을 구현★★★★★
    value = askinteger('감마','감마-->',minvalue=1.,maxvalue=10.)
    for i in range(inH):
        for k in range(inW):

            if int((inImage[i][k])**1/value) > 255:
                outImage[i][k]=255
            elif int((inImage[i][k])**1/value) < 0:
                outImage[i][k] = 0
            else:
                outImage[i][k]=int((inImage[i][k])**1/value)
    display()
...
#     for i in range(inH):
#         for k in range(inW):
#             outImage=np.array([[-1,-1,-1][-1,8,-1],[-1,-1,-1]])
#
#     display()
